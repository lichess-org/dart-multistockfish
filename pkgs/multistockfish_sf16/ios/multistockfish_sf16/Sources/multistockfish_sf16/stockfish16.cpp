...
// places that are invisible from Dart (most notably inside ThreadPool::set(0),
// which joins search threads and never returns if one of them ignored the stop
// flag). These few atomics are the only way to tell, from the outside, whether
// a start that never completed is stuck booting, stuck in the UCI loop, or
// stuck shutting down.
//
...
// True between entering stockfish_sf16_main() and returning from it. This is
// the re-entry guard: the engine's mutable state (Threads, TT, Options,
// Search::Limits) is process-global, so a second concurrent run would race the
// first over the very objects it is tearing down.
...
{
  Stockfish16::sfio::out() << std::flush;
...
namespace Stockfish16Init {
  ...
  int main(int argc, char* argv[]) {
    ...
    set_step("command_line");
    CommandLine::init(argc, argv);
    set_step("uci_options");
    UCI::init(Options);
    set_step("tune");
    Tune::init();
    set_step("psqt");
    ...
    set_step("endgames");
    Endgames::init();
    set_step("thread_pool");
    Threads.set(size_t(Options["Threads"]));
    set_step("search_clear");
    ...
    // Everything past this point is the teardown that can hang: set(0) waits
    // for the search to finish and then joins every thread.
    set_phase(SF_PHASE_SHUTTING_DOWN, "thread_pool_teardown");
    Threads.set(0);
    return 0;
  }
}
...
int stockfish_sf16_init()
{
  if (g_running.load(std::memory_order_acquire))
  {
    set_error(
        ...
        "It never returned from stockfish_sf16_main(); this process cannot host another engine.",
...
int stockfish_sf16_main()
{
  if (!g_pipes_ready.load(std::memory_order_acquire))
  {
    set_error("main: called before a successful stockfish_sf16_init()");
  ...
  {
    // Stockfish 16 keeps Threads, TT, Options and Search::Limits as process
    // globals. Running a second engine over them while the first is still
    // alive is undefined behaviour, so refuse instead.
    set_error(
  ...
  if (!Stockfish16::sfio::bind(CHILD_READ_FD, CHILD_WRITE_FD))
  {
  ...
  try
  {
    exitCode = Stockfish16Init::main(argc, argv);
  }
...
ssize_t stockfish_sf16_stdin_write(char *data)
{
  if (!g_pipes_ready.load(std::memory_order_acquire))
  {
    set_error("stdin_write: called before a successful stockfish_sf16_init()");
...
char *stockfish_sf16_stdout_read()
{
  if (!g_pipes_ready.load(std::memory_order_acquire))
  {
    set_error("stdout_read: called before a successful stockfish_sf16_init()");
    return NULL;
  }
